/*
Este archivo es donde tenemos toda la logica de login de nuestra aplicacion
tanto la creacion de usuarios, como el login con Google o Login con Email and Pass
*/
...
const index = () => {
	//Todas las variables y estados que vamos a necesitar
	const [email, setEmail] = useState(null);
	const [password, setPassword] = useState(null);
	const [error, setError] = useState(null);
	const [errorRegistro, setErrorRegistro] = useState(null);
	const rutas = useRouter();
	const { usuario, administrador, setUsuario, setAdministrador } = useContext(
		Contexto,
	);
	//Funcion asincrona para Login con Google.
	...
	//Funcion asincrona para el login con Email and Password
	const loginEmailAndPassword = async () => {
		try {
			...
			if (e.code === "auth/user-not-found") {
				setError(
					"No hay ningun usuario con ese Email en nuestra base de datos",
				);
			}
			...
			<Container>
				...
				<Row className='mt-3'>
					...
					<Col sm={12} md={6} lg={6}>
						...
						/>
						<Button
							onClick={registroUsuarioEmailPassword}
							className='btn btn-success btn-block mt-3'
						>
							REGISTRAR CUENTA
						</Button>
